...
- [x] **2.1** Add a `PipeSignature` section to `skills/mthds-vibe/references/vibe-cheat-sheet.md`: the signature syntax (design.md §1 reference block), the `signature_for` hint + its rules (not `"PipeSignature"`; it's a hint, not binding; multiplicity allowed), the operator-vs-controller decision, and the lenient (`--allow-signatures`) vs strict validate commands. The cheat sheet stays the syntax/field source of truth; the recursive loop itself lives in the SKILL body. → Added `### PipeSignature — a contract-only header (forward declaration)` subsection in §6 (after PipeFunc) + 3 signature entries in §10 Common Mistakes.
- [x] **2.2** Add `--allow-signatures` + `pending_signatures` to `templates/skills/shared/mthds-agent-guide.md.j2` (near the existing `validate bundle` / `--graph` docs, ~line 209/236/264): document the flag (forwarded verbatim to pipelex via `.allowUnknownOption()`), what lenient vs strict means, and that successful validate emits `pending_signatures`. → Added `## Lenient Validation — --allow-signatures` section (after Library Isolation) covering lenient/strict, the no-op-on-signature-free property, and reading `pending_signatures` via `--format json --error-format markdown`; updated the `validate bundle` row in the command-reference table.
- [x] **2.3** `make build && make check` to confirm references render and freshness/stale-reference checks stay green. → `make build` (all targets) + `make check` (freshness, ruff, pyright, both marketplaces, no-Claude-artifacts-in-Codex) all green. Confirmed new content rendered into `mthds/` and `mthds-codex/` shared guides + the static cheat sheet.
...
- [x] **4.2 — No fan-out in v1 (decision D1).** The layer loop runs **inline/serial on BOTH targets** — when a layer has multiple pending signatures, expand them one at a time in the orchestrator's own context. Do **not** add the `mthds-signature-expander` worker, the Agent-tool fan-out, or the `{% if platform != "codex" %}` fan-out block for v1 (that returns with the deferred Phase 3). Expansion instructions live **inline** in Step 2 — no `expand-signature` shared partial (single consumer). Keep the loop expressed so re-adding Claude fan-out later is a localized template change.
- [x] **4.2b — Concept-collision prevention (decision D5).** In Step 2, before introducing a new intermediate concept, **check it isn't already declared in the assembled library / pending set** and derive a unique code (namespace by the parent pipe code). Serial execution makes this nearly free; `ConceptLibraryError` stays the loud backstop. (Parallel-sibling collision handling returns with fan-out — see [wip/deferred-issues.md](wip/deferred-issues.md).)
- [x] **4.3 — Frontmatter + identity.** Keep the `mthds-vibe` name/slot. Update the `description` to reflect top-down, valid-at-every-step refinement. **Preserve `disable-model-invocation: true` for non-Codex** (the `{% if platform != "codex" %}` block already in the template) — this stays explicit-invocation-only.
- [x] **4.4 — Lazy-concepts + invariants prose.** Encode the core rules in the body: contract stability (concept identity, not byte-string), `main_pipe` frozen after Layer 0, concepts refine lazily (structure only when a consumer reads a field), additive writes (one concrete per file, headers persist), backlog = `{signatures} − {concretes}` via `pending_signatures`.
- [x] **4.5** `make build` (prod + dev + codex) + `/reload-plugins`. Both renders are **inline-serial** (no fan-out in v1, D1). Inspect the rendered `mthds/skills/mthds-vibe/SKILL.md` (has `disable-model-invocation: true`) and `mthds-codex/skills/mthds-vibe/SKILL.md` (no `disable-model-invocation`, no `allowed-tools` — the `check_codex_no_claude_artifacts` guard). The only per-target difference is the frontmatter, not the loop. → `make build` all targets + `make check` all scopes green. Verified frontmatter diff (prod: `disable-model-invocation: true` + `allowed-tools`; codex: neither). Body diff confirms the recursive loop renders **identically** on both targets — the only body deltas are inside the shared `preamble.md.j2` platform blocks (env-check script + `CODEX_CONFIG_NEEDS_SETUP`), not my rewrite. `/reload-plugins` deferred to the Phase 6.3 dogfood session.

### ⛔ CHECKPOINT 4 — skill rewritten and renders per target (HARD STOP)

Verification:
- [x] `make build && make check` green. → all scopes (shared freshness, ruff, pyright, Claude marketplace, Codex packaging + no-Claude-artifacts) pass.
- [x] Claude render has ~~the fan-out +~~ `disable-model-invocation: true`; Codex render is inline-serial + has neither `disable-model-invocation` nor `allowed-tools` (the `check_codex_no_claude_artifacts` guard). → **No fan-out in v1 (D1)** — the checkpoint's "fan-out" mention is superseded; both targets render inline-serial. Frontmatter confirmed: prod has `disable-model-invocation: true` + `allowed-tools` block; codex has neither.
- [x] A quick read-through of both renders for coherence (no dangling single-pass leftovers, no broken includes). → grep for `single pass`/`fan-out`/`sketch the shape`/`expand-signature`/`sub-agent` → **none**; preamble include rendered (env-check markers present); full Step 0–4 + How-it-works + Invariants + Autonomy + Reference headers present.

Record before continuing (filled 2026-06-08):

- **Summary of the new Step 0–4 body + any deviations from design §4.1:** Replaced the single-pass body of `templates/skills/mthds-vibe/SKILL.md.j2` entirely. New structure: title + intro (top-down stepwise refinement) → **Scope** (now lists `PipeSignature` as emittable; scope limits unchanged) → **How it works — read this first** (library/additive/invariant/operation/backlog, points at the cheat sheet) → **Step 0** env-check (unchanged include) → **Step 1** capture the whole job as one signature, write the root `bundle.mthds` (domain/description/main_pipe/system_prompt + fully-specified boundary concepts + top `PipeSignature` header), validate leniently (`--allow-signatures --graph`), announce the contract in one line (only interactive step) → **Step 2** drain `pending_signatures` serially via `--allow-signatures --graph --format json`, with a dedicated "To expand one signature" subsection (operator-vs-controller, add exactly one `<code>.mthds` file, `domain`-only member files, explicit inputs/output reconciled by concept identity, **collision-prevention check before introducing a concept**, lazy concept structuring) and an "If validation fails after an expansion" subsection (contract-mismatch → conform the **definition**, never the header; other errors → fix the added file) → **Step 3** strict validate (`--graph`) = runnable → **Step 4** deliver (input schema, never write `inputs.json`, point at `dry_run.html`, dry-run + `/mthds-inputs`) with the **early-stop scaffold variant** → **Invariants & rules** → **Autonomy** → **Reference**. **No deviations from design §4.1** beyond the locked v1 scope (no fan-out / no worker / no `expand-signature` partial — D1; expansion instructions live inline in Step 2 per the decision).
- **Confirmed per-target render differences (fan-out, frontmatter):** No fan-out on either target (D1). Only target-specific content: (1) **frontmatter** — prod/dev get `disable-model-invocation: true` + the `allowed-tools` block (via the `{% if platform != "codex" %}` gates in the template + `frontmatter.md.j2`); codex gets neither. (2) the **shared `preamble.md.j2`** platform blocks (codex env-check script path + `CODEX_CONFIG_NEEDS_SETUP` status). The recursive-loop body itself is **byte-identical** across targets (verified by diffing bodies after frontmatter — the only deltas are inside the preamble include).
- **Decisions made while writing the prose that aren't in design.md:** (a) Per-layer validate target = the **root `bundle.mthds`** (not the just-added file) for the skill's own explicit validate — simplest stable target; D2 guarantees any member validates the whole library, so this is a readability choice, not a correctness one. (b) Per-layer command pins **`--allow-signatures --graph --format json`** together (design §2.4 shows `--allow-signatures --graph`; TODOS Step 2 shows `--format json`) so the agent reads `pending_signatures` from stdout JSON *and* refreshes `dry_run.html` each layer in one call. Did **not** add `--error-format markdown` here (that pairing is the *hook's* concern — the skill agent reads errors directly, JSON or markdown both fine). (c) Folded the "expand one signature" job into a tight Step 2 subsection rather than a separate top-level section, to avoid over-explaining (single consumer, D1).
- **Update `docs/` (and the design doc's status) to reflect the shipped skill:** **Deferred to Phase 6.5** (the dedicated docs step), per the plan — `docs/build-targets.md`/`codex-vs-claude-hooks.md` updates and design.md §7 status flips are batched there after the end-to-end dogfood. Flagging here so it isn't lost.
...
- [x] **5.1** In `mthds-js/src/agent/commands/codex-hook.ts`, add `--allow-signatures` to `runPipelexValidate()` (the args array at ~line 285): `["validate","bundle",file,"-L",libraryDir,"--allow-signatures"]`. → Done. Also updated the `runPipelexValidate` doc comment (lenient = parity with Claude hook; no-op on signature-free; strict gate lives in skill finalize + `run`) and the Stage 3 call-site inline comment.
- [x] **5.2 — DEFERRED for v1 (nudge skipped, per plan recommendation).** Shipped **only 5.1** (`--allow-signatures`, **no** `--format json`, **no** success-path nudge). This sidesteps the error-format coupling entirely: the Codex hook still only inspects validate output **on failure** and errors stay **markdown-on-stderr**, so `classifyStage3Result()` is byte-for-byte unchanged. The orchestrator skill already tracks `pending_signatures` itself. Codex leftover-signature nudge logged as a follow-up in [wip/deferred-issues.md](wip/deferred-issues.md).
- [x] **5.3** Tests. **Deviation from plan:** the existing DI tests in `codex-hook.test.ts` mock at the `runPipelexValidate` boundary (`{exitCode, stderr}`), so they can't observe the argv where `--allow-signatures` lives. Added a **new** focused test file `mthds-js/tests/unit/agent/codex-hook-validate.test.ts` that mocks `node:child_process` (via `vi.hoisted` + `vi.mock`, the repo's established pattern from `pipelex-runner.test.ts`) and exercises the real `runPipelexValidate` wrapper — asserting the exact argv `["validate","bundle",<file>,"-L",<dir>,"--allow-signatures"]`, plus passthrough of non-zero exit/stderr and the spawn-error→127 mapping. Exported `runPipelexValidate` for this (consistent with the file's other "exported for testability" helpers). Existing `codex-hook.test.ts` unchanged and still green.
- [x] **5.4** Build/lint mthds-js per its own repo conventions. → `make check` (= `npm run build` (tsc) + `vitest run`) green: build clean, **775 tests pass** (44 files), incl. the new `codex-hook-validate.test.ts` (3) and all prior codex-hook tests. **Host global `mthds-agent` left at npm 0.9.0** (unchanged) — the local change rides in `dist/agent-cli.js` and is exercised here via `node dist/agent-cli.js codex hook`; Phase 6.2 Docker integration installs mthds-js from the mount so the dev-target hook picks it up there. (Did **not** `npm link` the host global, to avoid affecting unrelated host work.)

### ⛔ CHECKPOINT 5 — Codex hook at parity

Verification:
- [x] mthds-js unit tests pass. → `make check` green (build + 775 tests, 44 files).
- [x] A signature-containing bundle edited under the Codex `apply_patch` matcher is not blocked by Stage 3. → Real-CLI: ran the **built** hook (`node dist/agent-cli.js codex hook`) with an `apply_patch` payload touching the signature-containing `/tmp/mthds-smoke/lib/make_brief.mthds` → **exit 0, empty stdout/stderr (silent pass = NOT blocked)**. Contrast confirms the flag is the cause: same file via `pipelex-agent validate bundle … -L …` is **exit 1** ("depends on PipeSignature placeholders that have no implementation") **without** the flag and **exit 0 with** `--allow-signatures`. Bonus regression: a semantically-broken bundle under `--allow-signatures` still emits `{"decision":"block", … error_domain: input …}` → error path (markdown-on-stderr classifier) unaffected.
- [x] **internal-tools Docker integration suite** (workspace-rule obligation: I modified `mthds-js/`; the suite installs mthds-js from the mount so it exercises this change). **PASS (2026-06-08, Docker up):** `make -C ../internal-tools build` (both images built) then `make -C ../internal-tools agent-test` → `# ALL integration test scripts passed` for **both** fresh-install and upgrade-from-old. (Benign orphan-container warnings `cac-carol-dev`/`cac-nelly` from unrelated harnesses.) This also pre-completes the bulk of Phase 6.2.

Record before continuing (filled 2026-06-08):
- **mthds-js version after the change + its pipelex pin:** Package version **unchanged at `0.9.0`** (the version bump is deferred to Phase 6.4's floor coordination — see D4). Pipelex pin in the working tree is **`>=0.31.0`** (`src/agent/binaries.ts:41` `PIPELEX_PKG.version_constraint`) — but heads-up: that is a **pre-existing uncommitted edit, NOT from Phase 5**. HEAD still has `>=0.30.2` (the committed v0.9.0 floor); the working-tree bump to `>=0.31.0` predates this session and matches the Phase 0 note (line 92). Phase 5 did **not** touch `binaries.ts`; it shows in `git status` only because of that prior uncommitted edit. Phase 6.4 takes the pin to `>=<Part A+B release>`. The Phase 5 change itself is documented under a new `## [Unreleased]` section in `mthds-js/CHANGELOG.md` (no release tooling parses the changelog; Phase 6.4 will rename it to the cut version).
- **Whether 5.2 (nudge) was done or deferred:** **Deferred.** Only `--allow-signatures` shipped — no `--format json`, no success-path `pending_signatures` nudge — so the Codex error-format coupling stays closed and `classifyStage3Result()` is untouched. Logged as a follow-up in `wip/deferred-issues.md`.
- **Any version-coordination note between mthds-js and the plugin's `min_mthds_version`:** None forced now. This Codex-hook change does **not** itself require a floor bump — `--allow-signatures` is forwarded verbatim to `pipelex-agent` via the args array, and the *runtime* support for it (Part A+B reconciliation) is the gating dependency, not the mthds-agent surface. The `min_mthds_version` bump still happens at Phase 6.4, coordinated with: (a) the pipelex release carrying Part A+B, (b) bumping `binaries.ts:41` `version_constraint` to `>=<that pipelex version>` + the mthds-js package version, (c) `/bump-mthds-version` for the plugin floor. See the Phase 0 / D4 notes.
...
- `templates/skills/mthds-vibe/SKILL.md.j2` — ✅ Phase 4: single-pass body replaced by the recursive Step 0–4 flow (How-it-works, Step 1 root signature, Step 2 serial `pending_signatures` drain with inline expand-one-signature + D5 collision prevention + contract-mismatch recovery, Step 3 strict finalize, Step 4 deliver + early-stop). Updated `description`; preserved `disable-model-invocation: true` (non-codex). No fan-out / no `agents/` (D1). Renders identically on both targets (only frontmatter + shared preamble differ).
- `skills/mthds-vibe/references/vibe-cheat-sheet.md` — ✅ Phase 2.1: added `### PipeSignature` subsection (syntax, `signature_for` rules, operator-vs-controller, header↔definition contract by concept identity, lenient vs strict validate) + 3 signature mistakes in §10.
- `templates/skills/shared/mthds-agent-guide.md.j2` — ✅ Phase 2.2: added `## Lenient Validation — --allow-signatures` section (lenient/strict, no-op on signature-free, `pending_signatures` via `--format json --error-format markdown`); updated `validate bundle` table row.
...
- mthds-js — ✅ Phase 5 (v1: `--allow-signatures` only, nudge deferred):
  - `src/agent/commands/codex-hook.ts` — added `--allow-signatures` to `runPipelexValidate()`'s argv; exported the wrapper for testability; updated its doc comment + the Stage 3 call-site comment.
  - `tests/unit/agent/codex-hook-validate.test.ts` (**new**) — `vi.hoisted`+`vi.mock("node:child_process")` test asserting the lenient argv shape (+ exit/stderr passthrough, spawn-error→127). `codex-hook.test.ts` left unchanged.
  - `CHANGELOG.md` — new `## [Unreleased]` section documenting the lenient Codex Stage 3 change.
  - **Not changed:** `binaries.ts` pipelex pin (`>=0.31.0`) and package `version` (`0.9.0`) — both deferred to Phase 6.4 floor coordination.
