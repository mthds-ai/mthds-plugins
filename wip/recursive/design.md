...
- **Fan-out target scope.** Parallel fan-out ships on **Claude** (the `agents/` worker + Agent tool); under **Codex** the layer loop runs **inline/serial** for v1 (§4.4). Codex sub-agents exist but can't be bundled in the plugin (no `agents` manifest field) — they'd be installed as `.codex/agents/*.toml` via `mthds-agent codex apply-config`. That Codex fan-out is a deferred follow-up, not part of this rollout.
