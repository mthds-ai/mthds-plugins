...
def template_tree(tmp_path: Path) -> Path:
    """Create a minimal skill directory with a .j2 template, shared include, and target configs."""
    ...
    # Create target configs for generate() / check_freshness()
...
class TestRenderTemplates:
    def test_renders_include(self, template_tree: Path) -> None:
        ...
        results = render_templates(skills_dir, DEFAULT_VARS)
        skill_output = skills_dir / "mthds-test" / "SKILL.md"
    ...
    def test_renders_shared_templates(self, template_tree: Path) -> None:
        skills_dir = template_tree / "skills"
        results = render_templates(skills_dir, DEFAULT_VARS)
        guide_output = skills_dir / "shared" / "mthds-agent-guide.md"
    ...
    def test_missing_include_raises(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(SystemExit, match="include file not found"):
            render_templates(skills_dir, DEFAULT_VARS)
    ...
    def test_missing_skills_dir_raises(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(SystemExit, match="Skills directory not found"):
            render_templates(missing_dir, DEFAULT_VARS)
    ...
    def test_include_skills_filter(self, template_tree: Path) -> None:
        ...
        results = render_templates(skills_dir, DEFAULT_VARS, include_skills=["mthds-test"])
        skill_names = {path.parent.name for path in results if path.parent.name != "shared"}
    ...
    def test_template_vars_injected(self, template_tree: Path) -> None:
        ...
        skills_dir = template_tree / "skills"
        skill_dir = skills_dir / "mthds-var"
        skill_dir.mkdir()
        (skill_dir / "SKILL.md.j2").write_text("Version: {{ min_mthds_version }}\n")

        custom_vars = {**DEFAULT_VARS, "min_mthds_version": "9.9.9"}
        results = render_templates(skills_dir, custom_vars)
        output_path = skills_dir / "mthds-var" / "SKILL.md"
