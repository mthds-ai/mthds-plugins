...
Run `make check` (which verifies shared refs, shared file existence, version
consistency, and template freshness — ensuring generated `SKILL.md` files match
their `.j2` sources). This is the gate — if it fails, stop and report the errors
so they can be fixed before retrying. Do not proceed past this step on failure.
