...
- **Plugin (Claude Code plugin):** Detected by `mthds-agent update-check` when the `plugin` entry in `UPGRADE_AVAILABLE` shows `outdated` or `missing`. Upgraded via `claude plugin install mthds-sandbox@mthds-plugins`.
...
**For the Claude Code plugin (`mthds-sandbox@mthds-plugins`):**

```bash
claude plugin install mthds-sandbox@mthds-plugins
```
